    ...
    if (it != mAccount.end()) {
        if (order.instType == SPOT || order.instType == MARGIN) {
            if (order.direction == DT_LONG) {
                ...
                LOG_INFO("FundVerify strategyAccountId:{} instrumentKey:{} instRight:{} totalAmount:{} floatAmount:{} frozenAmount:{} availableAssetAmount:{} requireAmount:{}", order.strategyAccountId, order.instrumentKey, info.quote, ass.totalAmount, ass.floatAmount, ass.frozenAmount, availableAssetAmount, requireAmount);
            ...
            if (order.direction == DT_LONG) {
                ...
                LOG_INFO("FundVerify strategyAccountId:{} margin:{} instrumentKey:{} order.volume:{} pos.shortPosition:{} totalAmount:{} floatAmount:{} frozenAmount:{} positionValue:{} availableAssetAmount:{} requireMargin:{} requireAmount:{}", order.strategyAccountId, info.margin, order.instrumentKey, order.volume, pos.shortPosition, ass.totalAmount, ass.floatAmount, ass.frozenAmount, ass.positionValue, availableAssetAmount, requireMargin, requireAmount);
                if (requireAmount < stra::MIN_FLOAT) {
                    return true;
                } else {
                    return false;
                ...
                LOG_INFO("FundVerify strategyAccountId:{} margin:{} instrumentKey:{} order.volume:{} pos.longPosition:{} pos.shortPosition:{} totalAmount:{} floatAmount:{} frozenAmount:{} positionValue:{} availableAssetAmount:{} requireMargin:{} requireAmount:{}", order.strategyAccountId, info.margin, order.instrumentKey, order.volume, pos.longPosition, pos.shortPosition, ass.totalAmount, ass.floatAmount, ass.frozenAmount, ass.positionValue, availableAssetAmount, requireMargin, requireAmount);
                if (requireAmount < stra::MIN_FLOAT) {
                    return true;
                } else {
                    return false;
        ...
        LOG_INFO("FundVerify cannot find accountId:{}", order.strategyAccountId);
    }
    return false;
}
